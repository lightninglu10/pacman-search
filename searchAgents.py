...
def foodHeuristic(state, problem):
    ...
    bigcluster = []
    """
    def findclust((x, y), foodcluster = [], dictionary = {}):
        x, y = food_coordinate
        if dictionary.has_key(x, y):
            continue
        else:
            dictionary[(x, y)] = True
            if (x+1, y) in listgrid:
                if dictionary.has_key((x+1, y)):
                    continue
                else :
                    foodcluster.append((x+1, y))
                    foodcluster = findclust((x+1, y), foodcluster, dictionary)
            if (x, y+1) in listgrid:
                if dictionary.has_key((x, y+1)):
                    continue
                else:
                    foodcluster.append((x, y+1))
                    foodcluster = findclust((x+1, y), foodcluster, dictionary)
            if (x-1, y) in listgrid:
                if dictionary.has_key((x-1, y)):
                    continue
            ...
            if (x, y-1) in listgrid:
                if dictionary.has_key((x, y-1)):
                    ...
                    foodcluster.append((x, y-1))
                    foodcluster = findclust((x, y-1), foodcluster, dictionary)
        return foodcluster
        """
    for food_coordinate in listgrid:
        #multicluster.append(findclust((food_coordinate[0], food_coordinate[1]), dicts))
        #for cluster in multicluster:
...
def greedy(state, problem):
    position, foodGrid = state
    distances = []
    for food_coordinate in foodGrid.asList():
        distances.append(util.manhattanDistance(food_coordinate, position))
    if not distances:
        return 0
